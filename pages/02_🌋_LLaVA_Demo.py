...
st.set_page_config(page_title="LLaVA Playground", page_icon="🌋",
...
def main():
    icon("🌋")
    st.subheader("LLaVA 1.6 Playground", divider="grey", anchor=False)
    ...
    with col2:
        ...
        if uploaded_file is not None:
            ...
            if user_input := st.chat_input(
                ...
                container2.chat_message("user").markdown(user_input)

                image_base64 = img_to_base64(image)
                API_URL = "http://localhost:11434/api/generate"
                headers = {"Content-Type": "application/json",
                           "Accept": "application/json"}
                data = {"model": "llava", "prompt": user_input,
                        "images": [image_base64]}

                with container2.chat_message("assistant"):
